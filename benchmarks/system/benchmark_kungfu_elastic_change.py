...
ssgd_opt = SynchronousSGDOptimizer(opt)
from kungfu.tensorflow.optimizers import PairAveragingOptimizer
pair_opt = PairAveragingOptimizer(opt, fuse_requests=args.fuse)
...
loss = loss_function()
ssgd_train_op = ssgd_opt.minimize(loss)
pair_train_op = pair_opt.minimize(loss)
...
with tf.Session(config=config) as session:
    ...
    run(session, bcast_op, ssgd_train_op, pair_train_op)
print('stopped')
